...
%         maxPooling2dLayer(3, 'Stride',2)
    ...
    layers(1,1).Mean = 123*ones(1,1,3);
    ...
    layers(3,1).TrainedMean = 1e-4*ones(1,1,32);
    layers(3,1).TrainedVariance = 0.0040*ones(1,1,32);
    layers(3,1).Offset = zeros(1,1,32);
    layers(3,1).Scale = ones(1,1,32);
    ...
    layers(11,1).Weights = zeros(C,64); 
...
%     end
    for ii=1:K
     ...
     for iter_glob = 1:T_glob
        ...
        [A_t,W_t,sigma_all] = small_eval_aux_data_multiclass_CIFAR(W_G,X_aux_all,Y_aux_all,epochs,0.01,C,net1);% for the aux-gradient method
        for ii=1:K
           ...
           % flag_plot=1;
            [W_local,net] = small_NN_train_local_CIFAR_multiclass(XTrain_10,YTrain_10,epochs,N_all(ii),W_G,flag_plot,C);
    ...
    save('CIFAR_10_exact.mat','p_true','p_est_org','p_est_aux_grad','p_est_aux_W_grad','p_est_compare','p_est_compare_mod','p_est_soft_label','epochs','B','T_glob');
    
    
end

%%
ip_filename = 'CIFAR_10_exact.mat';
op_filename = 'CIFAR_10_exact';
[mean_KL,mean_MSE] = KL_divergence_and_MSE(ip_filename,op_filename,10,10,1,10);
