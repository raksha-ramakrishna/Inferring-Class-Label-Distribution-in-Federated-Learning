...
for fl=1:FL
    ...
    for iter_glob = 1:T_glob
        ...
        W_local_all=[];
        [A_t,W_t,sigma_all] = small_eval_aux_data_multiclass(W_G,X_aux_all,Y_aux_all,epochs,0.01,C);% for the aux-gradient method
        for ii=1:K
            ...
            [W_local,net] = small_NN_train_local_MNIST_multiclass(XTrain_10,YTrain_10,epochs,B,W_G,flag_plot,C);
            %W_local_all(ii,1:8)=W_local;
            b_updates(:,ii,iter_glob)= W_local{1,6};
            W_local_all = [W_local_all;W_local];
            % regular method:
            p_est = regular_prop_est(W_G{1,6},W_local{1,6},epochs,B,N_all(ii),0.01);
            % aux grad method:
    ...
    toc
    disp(fl)
    save('MNIST_E5_sampling_corrected.mat','p_true','p_est_org','p_est_aux_grad','p_est_aux_W_grad','p_est_soft_label','p_est_compare','p_est_compare_mod','epochs','B','T_glob');
    
end
%%

ip_filename='MNIST_E5_sampling_corrected.mat';
op_filename = 'MNIST_sampled_corrected';
