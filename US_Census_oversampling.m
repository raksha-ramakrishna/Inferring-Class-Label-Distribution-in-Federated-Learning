...
p_true = zeros(C,K,T_glob,FL,2);
p_est_org = zeros(C,K,T_glob,FL,2);
p_est_aux_grad = zeros(C,K,T_glob,FL,2);
p_est_aux_W_grad  = zeros(C,K,T_glob,FL,2);
p_est_soft_label = zeros(C,K,T_glob,FL,2);
p_est_compare = zeros(C,K,T_glob,FL,2);
p_est_compare_mod = zeros(C,K,T_glob,FL,2);
%%
...
for fl=1:FL
    ...
    randperm_L_all = [];
    randperm_L_oversample=[]; %oversampled vals
    N_all_o=zeros(K,1);
    
    for ii=1:K
        N_all(ii) = randi([100,LB/2],1); %size is between 1 and 3000
        [N_class,N_new] = random_oversampling_prob(N_all(ii),C);
        randperm_L=[];
        randperm_L_o=[];
        n_c=[];
        ...
        for cc=1:C
            ...
            if(N_class(cc)==min(N_class))
                oversample_ind = indices(ind_cc);
            end
        ...
        % only the oversampling part:
        n_sample = max(n_c)-min(n_c);
        
        ind_cc_oversample = randi([1,length(oversample_ind)],n_sample,1); %sampling with replacement
        oversample_ind_add = client_indices(oversample_ind(ind_cc_oversample));
        randperm_L_oversample = [randperm_L_oversample; [randperm_L;oversample_ind_add] ];
        N_all_o(ii) = sum(n_c)+length(ind_cc_oversample);
    ...
    for gg=1:2
        if(gg==1)
            randperm_L_g = randperm_L_all;
            N_all_g = N_all;
        else
            randperm_L_g = randperm_L_oversample;
            N_all_g = N_all_o;
        end
        for iter_glob = 1:T_glob
            ...
            for ii=1:K
                ...
                if(ii>1)
                    randperm_L = randperm_L_g(sum(N_all_g(1:ii-1))+1:sum(N_all_g(1:ii)));
                else
                    randperm_L = randperm_L_g(1:N_all_g(1));
                ...
                %% save values:
                p_true(:,ii,iter_glob,fl,gg) = p_org; %100 different iterations of FL
                p_est_org(:,ii,iter_glob,fl,gg) = p_est;
                p_est_aux_grad(:,ii,iter_glob,fl,gg) = p_aux;
                p_est_aux_W_grad(:,ii,iter_glob,fl,gg) = p_est_aux_W;
                p_est_soft_label(:,ii,iter_glob,fl,gg) = p_soft;
                p_est_compare(:,ii,iter_glob,fl,gg) = p_comp;
                p_est_compare_mod(:,ii,iter_glob,fl,gg) = p_comp_mod;
            end
            %toc
    ...
    toc
    save('new_comparison_US_Census_oversampling.mat','p_true','p_est_org','p_est_aux_grad','p_est_soft_label','p_est_compare','p_est_compare_mod','p_est_aux_W_grad','epochs','B','T_glob','b_updates','b_glob');
    
end
%%
ip_filename='new_comparison_US_Census_oversampling.mat';
op_filename = 'US_Census_oversampled';
[mean_MSE_1,mean_MSE_2] = KL_divergence_and_MSE_oversample(ip_filename,op_filename,2,10,20,10);
